...
class TestMetricsSemanticMetadataStoreIntegration:
    def test_complete_metric_semantic_metadata_workflow(self, store):
        ...
        http_latency_id = store.index_metadata(
            namespace,
            {
        ...
        error_rate_id = store.index_metadata(
            namespace,
            {
        ...
        throughput_id = store.index_metadata(
            namespace,
            {
        ...
        latency_results = store.search_metadata(
            "request latency and response time", n_results=5
        )
        ...
        updated_id = store.index_metadata(
            namespace,
            {
                "metric_name": "http.request.duration.p99",
                "type": "histogram",
        ...
        assert updated_id == http_latency_id, (
            "Should return same document ID for upsert"
        )

        # Verify the update
        updated_results = store.search_metadata(
            "improved accuracy latency", n_results=1
        )
