...
Provide your analysis in the structured format."""
    ...
    def explain_query(self, promql_query: str) -> str:
        ...
        This is a convenience method that just explains the query without
        comparing to any original intent.
        ...
        Raises:
            SemanticValidationError: If explanation fails
        """
        if not promql_query or not promql_query.strip():
            raise SemanticValidationError("Query cannot be empty")

        prompt = f"""Explain what this PromQL query does in plain language:
...
Describe:
1. What metric(s) it queries
2. What filters are applied
3. What time window it uses
4. What aggregations or functions are applied
5. What the overall query computes
...
Provide a clear, technical explanation."""
        ...
        try:
            agent = self._get_agent()
            result = agent.run_sync(prompt)
            return result.output.actual_query_behavior
        except Exception as e:
            logger.error(f"Query explanation failed: {e}", exc_info=True)
            raise SemanticValidationError(
