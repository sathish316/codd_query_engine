...
from codd_engine.querygen_engine.custom_tool.logql_validator_tool import (
    LogQLValidatorTool,
)
...
class LogQLQueryGeneratorAgent:
    ...
    def _init_agent(self):
        """Initialize the LogQL query generator agent with validation tool."""
        self.agent = (
            AgentBuilder(self.config_manager)
            .set_system_prompt_keys(["logql_query_generator_agent_instruction"])
            .name("logql-query-generator-agent")
            .add_instructions_manager(self.instructions_manager)
            .add_model_manager()
            .instruction(
                "logql_query_generator_agent_instruction",
                expand_path("$HOME/.codd/prompts/agent/logs/LOGQL_QUERY_GENERATOR_AGENT_INSTRUCTIONS.md"),
            )
    ...
    async def generate_query(self, intent: LogQueryIntent) -> QueryGenerationResult:
        ...
        2. Use the validate_logql_query tool to check the query
        ...
        try:
            ...
            raise QueryGenerationError(
                f"Failed to generate LogQL query for '{intent.description}': {e}"
            ) from e
    ...
    def _format_generation_prompt(self, intent: LogQueryIntent) -> str:
        ...
        prompt = f"""Generate a valid LogQL query for the following log query intent:
...
2. Use the `validate_logql_query` tool to validate your query
...
Generate the best possible LogQL query for the intent, then validate it using the tool."""
