...
from codd_dal.metrics.metrics_semantic_metadata_store import (
...
def initialize_clients(args):
    """
    Initialize Redis and ChromaDB clients using CoddConfig models.
...
if __name__ == "__main__":
    ...
    # uv run -m codd_jobs.metrics_semantic_indexer_main --namespace "default:beer-service" --promql-url "http://localhost:9090" --redis-host "localhost" --redis-port 6380 --chromadb-host "localhost" --chromadb-port 8000 --batch-size 10 --limit 400 --skip-if-present --exclude-pattern "^(go|prometheus)"
    # To run the job in query mode
    # uv run -m codd_jobs.metrics_semantic_indexer_main --namespace "default:beer-service" --promql-url "http://localhost:9090" --redis-host "localhost" --redis-port 6380 --chromadb-host "localhost" --chromadb-port 8000 --query beer --query-limit 5
    main()
